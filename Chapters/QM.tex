...
\begin{align}
| s, m \rangle &= \sum_{m=m_1+m_2} C_{m_1m_2m}^{s_1s_2s}| s_1, m_1 \rangle| s_2, m_2 \rangle \\ | s_1, m_1 \rangle| s_2, m_2 \rangle &= \sum_{s} C_{m_1m_2m}^{s_1s_2s}| s, m \rangle
\end{align}
...
From the \textbf{Time-independent perturbation theory} at a first order approximation gives the correction energy and wave function as
\begin{align}
	| \psi_n^{(1)} \rangle = \sum_{\ell \neq n} | \psi_\ell^{(0)} \rangle \frac{\langle \psi_\ell^{(0)}|H^1| \psi_n^{(0)} \rangle }{E_n^{(0)}-E_\ell^{(0)}} \hspace{2cm} E_n^{(1)}=\langle \psi_n^{(0)}|H^1| \psi_n^{(0)} \rangle  
\end{align}